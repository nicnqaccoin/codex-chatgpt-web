...
export function installedLauncherCandidates({
  ...
  if (platform === "darwin") {
    ...
    // `windowsInstallLocation` is the injection point for the installer registry value, so an
    // explicit argument - including an empty one meaning "no registered install" - has to suppress
    // the lookup. Falling through on empty made the real registry answer for callers that had
    // supplied a complete fake environment, which is why this could only be exercised on a machine
    // where the launcher happened not to be installed.
